...
export default function RapGenerator() {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/generate-diss', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        ...
        <header className="relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center space-y-8">
            ...
            <p className="text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto px-4">
              Generate unique diss tracks! Get creative with our AI-powered
              diss track generator that combines diss tracks's legendary style with your ideas.
            </p>
            <AnimatedTooltipPreview />
            <p className="text-sm text-gray-600 dark:text-gray-400 max-w-2xl mx-auto px-4">Trusted by over 300,000 diss track enthusiasts worldwide.</p>
          </div>
        </header>
              ...
              <form onSubmit={handleFormSubmit} className="space-y-6">
                <div className="mb-8">
                  ...
                  <p className="text-neutral-600 text-sm max-w-sm mt-2 dark:text-neutral-300">
                    Enter your details to generate your unique diss track
                  </p>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <select
                      name="language"
                      className="w-full px-4 py-2 bg-zinc-800 rounded-md border border-zinc-700 focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition-colors"
                      required
                  ...
                  <div>
                    ...
                    <input
                      ...
                      name="target"
                      className="w-full px-4 py-2 bg-zinc-800 rounded-md border border-zinc-700 focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition-colors"
                      placeholder="Who or what are you dissing? (e.g., fake friends, haters)"
                      required
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">
                      Theme/Reason <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      name="theme"
                      className="w-full px-4 py-2 bg-zinc-800 rounded-md border border-zinc-700 focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition-colors"
                      placeholder="Why are you dissing them? (e.g., betrayal, jealousy)"
                      required
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">
                      Keywords <span className="text-neutral-400 text-xs ml-1">(Optional)</span>
                    </label>
                    <input
                      type="text"
                      name="keywords"
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">
                      Style <span className="text-neutral-400 text-xs ml-1">(Optional)</span>
                    </label>
                    <select
                      name="style"
                      className="w-full px-4 py-2 bg-zinc-800 rounded-md border border-zinc-700 focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition-colors"
                      defaultValue="classic-rap"
                    >
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">
                      Length <span className="text-neutral-400 text-xs ml-1">(Optional)</span>
                    </label>
                    <select
                      name="length"
                      className="w-full px-4 py-2 bg-zinc-800 rounded-md border border-zinc-700 focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition-colors"
                ...
                <div className="mt-8">
                  ...
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="w-full py-3 px-4 bg-yellow-500 hover:bg-yellow-600 text-black font-bold rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isLoading ? 'Creating Your Lyrics...' : 'Generate Diss Track'}
                  </button>
                </div>
              </form>
            </div>
